...
cdak $ print *, '&&& bad QM fails sanity check'
     ...
     $          POB.GT.1100.) then
                 if(tdo.lt.-103.15 .or. tdo .gt.46.83) then
C slm: Still try to derive Pmsl for surface obs using Ts instead of Tv
                   if(surf) then
                     if(dopmsl) then
                       if(ibfms(pmo_8).ne.0) then
C                        tv = tob + 273.16
                         zob=zob_8
                         pmsl_8=PMSL_fcn(pob,tob,zob)
                         bakp_8(1) = pmsl_8
                         bakp_8(2) = max(3,int(pqm_8))! qm>=3 b/c derived
                         bakp_8(3) = 1. ! pmin=1 for derived value
                         write(*,4002) stnid,bakp_8(3),bakp_8(2),tob
 4002 format('--> ID ',A8,' Pmsl missing - derived from Pstn, Ts, Z; ',
     $ 'PMIN = ',F4.1,' PMQ = ',F4.1,' tob = ',F7.2,'')
                         evnp = .true.
c Diagnostics for Pmsl values that are suspiciously high
                         if(pmsl_8.gt.1060) then
                           write(*,4003) stnid,pob,bakp_8(1)
 4003 format('--> ID ',A8,' Derived PMSL unrealistic; FLAG; ',
     $ 'POB = ',F7.2,' PMO = ',F7.2,'')
                         end if
                       end if
                     end if
...
c       liekly be made). Might also be able to be derived if no T rpted.
                    if(dopmsl) then
                      if(ibfms(pmo_8).ne.0) then
                        ...
                        bakp_8(2) = max(3,int(pqm_8))! qm>=3 b/c derived
                        bakp_8(3) = 1. ! pmin=1 for derived value
                        write(*,4000) stnid,bakp_8(3),bakp_8(2),tv,tob
 4000 format('--> ID ',A8,' Pmsl missing - derived from Pstn, Tv, Z; ',
     $ 'PMIN = ',F4.1,' PMQ = ',F4.1,' Tv = ',F7.2,' tob = ',F5.2,'')
                        evnp = .true.
c Diagnostics for Pmsl values that are suspiciously high
                        if(pmsl_8.gt.1060) then

...
#export PDY=20160810 # may need to hard set the date if /com/date/t06z has already been updated
tmmark=$1
if [ $tmmark -eq 00 ]; then
  ...
  network=ndas
  test $tmmark -eq 03 && CDATE=${PDY}03
  test $tmmark -eq 06 && CDATE=${PDY}00
  test $tmmark -eq 09 && CDATE=$(sh /nwprod/util/ush/finddate.sh $PDY d-1)21
  test $tmmark -eq 12 && CDATE=$(sh /nwprod/util/ush/finddate.sh $PDY d-1)18
fi

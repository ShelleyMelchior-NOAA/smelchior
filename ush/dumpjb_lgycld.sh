...
# Run dumpjb for dump group: lgycld
...
workdir=$stmp/$USER/dumpjb_lgycld_${network}.${PDY}${cyc}.$$
...
if [ "$network" = "gfs" -o "$network" = "gdas" ]; then
  export LALO=0
  CRAD=3.0
elif [ "$network" = "rap" -o "$network" = "nam" ]; then
...
# Dump nc4 format lgycld tanks
export TANK=/gpfs/dell2/emc/obsproc/noscrub/Sudhir.Nadiga/DCOMDIR/LGCLOUD/prod
#export TANK=$DCOMROOT/prod

echo $HOMEobsproc_dump/ush/dumpjb $CDATE $CRAD lgycld
$HOMEobsproc_dump/ush/dumpjb $CDATE $CRAD lgycld


# cp *lgycld.ibm to use filename format GSI is expecting
mkdir -p $stmp/$USER/com/prod/${2}.${PDY}
cp $workdir/lgycld.ibm $workdir/${2}.t${1}z.lgycld.tm00.bufr_d

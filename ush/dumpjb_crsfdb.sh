...
# Run dumpjb for dump group: crsfdb 
...
# export HOMEobsproc_shared_bufr_dumplist=/gpfs/dell1/nco/ops/nwprod/obsproc_shared/bufr_dumplist.v2.1.0
  export HOMEobsproc_shared_bufr_dumplist=/gpfs/dell2/emc/obsproc/noscrub/$USER/gitstatic/obsproc_shared_bufr_dumplist_RB-2.2.0
...
workdir=$stmp/$USER/dumpjb_crsfdb_${network}.${PDY}${cyc}.$$
...
export TANK_021212=/gpfs/dell2/emc/obsproc/noscrub/Mitchell.Weiss/for_Shelley

DTIM_latest_crsfdb=${DTIM_latest_crsfdb:-"+2.99"}

echo $HOMEobsproc_dump/ush/dumpjb $CDATE $CRAD crsfdb
$HOMEobsproc_dump/ush/dumpjb $CDATE $CRAD crsfdb


# cp *crsfdb.ibm to /com location from which dump archive can retrieve the files
# use filename format GSI is expecting
outdir=$stmp/$USER/com_nppcrisfsr/prod/${2}.${PDY}/${1}
mkdir -p $outdir
cp $workdir/crsfdb.ibm ${outdir}/${2}.t${1}z.crsfdb.tm00.bufr_d

...
#export PDY=20160810 # may need to hard set the date if /com/date/t18z has already been updated
tmmark=$1
if [ $tmmark -eq 00 ]; then
  ...
  network=ndas
  test $tmmark -eq 03 && CDATE=${PDY}15
  test $tmmark -eq 06 && CDATE=${PDY}12
  test $tmmark -eq 09 && CDATE=${PDY}09
  test $tmmark -eq 12 && CDATE=${PDY}06
fi

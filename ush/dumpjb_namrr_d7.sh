...
# Run dumpjb for dump group: goesfv, lgycld
...
workdir=/stmpp1/$USER/dumpjb_namrr_d7_${network}.${PDY}${cyc}.${tmmark}.$$
...
echo $HOMEobsproc_dump/ush/dumpjb $dumptime $CRAD goesfv lgycld
$HOMEobsproc_dump/ush/dumpjb $dumptime $CRAD goesfv lgycld

# copy *.ibm to name prep processing is expecting
mv $workdir/goesfv.ibm $workdir/${network}.t${cyc}z.goesfv.tm${tmmark}.bufr_d
mv $workdir/lgycld.ibm $workdir/${network}.t${cyc}z.lgycld.tm${tmmark}.bufr_d

export tstsp=$workdir/${network}.t${cyc}z.

mv $workdir/goesfv.out $workdir/goesfv.${network}.out
mv $workdir/lgycld.out $workdir/lgycld.${network}.out
